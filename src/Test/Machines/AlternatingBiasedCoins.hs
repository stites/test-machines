...
module Test.Machines.AlternatingBiasedCoins where
...
data Coin = EvenCoin | OddCoin
  deriving (Eq, Ord, Show)


coinProxy :: Proxy Coin
coinProxy = Proxy

instance SeriesGenerating Coin where
  type Symbol Coin = BinaryOutput
  initial = EvenCoin

  move EvenCoin Z = OddCoin
  move EvenCoin O = EvenCoin
  move OddCoin  Z = OddCoin
  move OddCoin  O = EvenCoin

  distribution EvenCoin = [ (0.75, Z), (0.25, O) ]
  distribution OddCoin  = [ (0.25, Z), (0.75, O) ]
